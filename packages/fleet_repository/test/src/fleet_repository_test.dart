  ...
  group('FleetRepository', () {
    ...
    group('Constructor', () {
      ...
      test('creates default dependencies when none provided', () {
        ...
        expect(defaultRepo.localApiType, equals('HiveFleetApi'));
        defaultRepo.dispose();
      });
    });
    ...
    group('Car fetching and caching', () {
      ...
      test('filterCarsByStatus returns filtered cached cars', () async {
        ...
        expect(result.every((car) => car.status == CarStatus.parked), isTrue);
        verify(() => localApi.fetchCars()).called(1);
      });
    ...
    group('Car streaming', () {
      ...
      test('watchCachedCars passes custom poll interval', () {
        ...
        verify(() => localApi.watchAllCars(pollInterval: customInterval))
            .called(1);
      });
      ...
      test('watchCachedCarLocation passes custom poll interval', () {
        ...
        verify(() => localApi.watchCarLocation(1, pollInterval: customInterval))
            .called(1);
      });
    });

    group('Car details', () {
      test('fetchAndCacheCarDetails gets from remote and caches locally',
          () async {
      ...
      test('fetchAndCacheCarDetails falls back to cache on remote failure',
          () async {
        when(() => remoteApi.fetchCarDetails(any()))
